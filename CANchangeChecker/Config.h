...
class Config : public ConsolePrint{
	...
	public :
		...
		void readConfig() {
			...
			std::cout << "Reading licznik.txt : ";
			std::string line;
			configFile.open(PATH_CONFIG_FILE);

			if (!configFile.is_open()) {
				// there is no point trying to create new file as function updateConfig do this anyway
				printColorMessage("WARNING","could not open licznik.txt file, it will be created from scratch", ConsolePrint::c_WARNING);
			}
			else {
				while (getline(configFile, line)) {
					...
					if (line.rfind("Liczba kompilacji projektu") != std::string::npos) {
						...
						{ 
							...
							printColorMessage("WARNING", "could not read compile time, it will be 0", ConsolePrint::c_WARNING);
		...
		void updateConfig() {
			...
			std::cout << "updating licznik.txt : ";
			configFileWrite.open(PATH_CONFIG_FILE);

			if (!configFileWrite.is_open()) {
				printColorMessage("ERROR","could not open licznik.txt file, please update it manually", ConsolePrint::c_WARNING);
			}
			else {
